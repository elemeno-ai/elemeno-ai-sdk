...
class TestMinioIngestion(unittest.TestCase):
    ...
    def test_download_file_from_remote_invalid_file_path(self):
        # Setup
        test_config = self.setupConfig()
        with patch("elemeno_ai_sdk.config.Configs.instance", return_value=test_config), patch(
            "elemeno_ai_sdk.cos.minio.MinioClient.bucket_exists"
        ) as existsmock, patch("elemeno_ai_sdk.cos.minio.MinioClient.get_object") as fget_object_mock, patch(
            "logging.error"
        ) as log_error_mock:
            ...
            params = DigestionParams(
                media_path_col="file_path", to_digest={"file_path": "non_existent.txt"}, remote_folder="test_folder"
            )
    ...
    def test_download_file_from_remote_invalid_bucket(self):
        ...
        with patch("elemeno_ai_sdk.config.Configs.instance", return_value=test_config), patch(
            ...
            ingestion = MinioIngestion()
            params = DigestionParams(
                media_path_col="file_path",
                to_digest={"file_path": "test.txt"},
    ...
    def test_local_folder_creation(self, mock_exists, mock_makedirs):
        ...
        with patch("elemeno_ai_sdk.config.Configs.instance", return_value=test_config), patch(
            ...
            ingestion = MinioIngestion()
            params = DigestionParams(
                media_path_col="file_path", to_digest={"file_path": "test.txt"}, remote_folder="test_folder"
            )
