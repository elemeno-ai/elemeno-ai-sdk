...
import pandas as pd
from elemeno_ai_sdk.ml.features.ingest.sink.minio_ingestion import MinioIngestion, MediaColumn
...
class TestMinioIngestion(unittest.TestCase):
  ...
  def test_io_batch_digest_media_files_multiple_columns(self, mock_pool, mock_cpu_count):
    ...
    feature_table_name = "test_feature_table"
    to_digest = pd.DataFrame({"media_col_1": ["file_path_1", "file_path_2"], "media_col_2": ["file_path_3", "file_path_4"]})
    media_columns = [MediaColumn(name="media_col_1", is_upload=False), MediaColumn(name="media_col_2", is_upload=True)]
