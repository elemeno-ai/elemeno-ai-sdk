...
def get_and_upload_dask(to_get: Dict) -> None:
    ...
    client = MinioClient(host="minio.minio:9000", access_key="elemeno", secret_key="minio123", use_ssl=False)
    try:
        ...
        if content_type.startswith("image"):
            ...
            try:
                ...
                # print(e)
                client.put_object(
                    "elemeno-cos", f"binary_data_parallel/{offer_id}/{position}_{media_id}.{media_extension}", st
                )
                logging.error(
                    "uploaded path: " + f"binary_data_parallel/{offer_id}/{position}_{media_id}.{media_extension}"
                )
...
class ElasticToCos:
    def __init__(self, client: MinioClient) -> None:
        ...
        self._es = Elasticsearch(hosts=[elastic_cfg.host], http_auth=(elastic_cfg.user, elastic_cfg.password))
        dask_client = Client(
            address="a6304c020a60a460cb97247a70f88d6f-805997406.us-east-1.elb.amazonaws.com:8786", timeout=300
        )
