...
class RedshiftIngestion(Ingestion):
    ...
    def ingest(
        ...
        ft: FeatureTable,
        renames: typing.Optional[typing.Dict[str, str]] = None,
        expected_columns: typing.Optional[typing.List[str]] = None,
    ) -> None:
        """ "
        ...
        try:
            with open(schema_file_path, mode="r", encoding="utf-8") as schema_file:
                ...
                for name, prop in jschema["properties"].items():
                    ...
                    table_schema.append(
                        {"name": name, "type": FeatureType.from_str_to_bq_type(prop["type"], format=fmt).name}
                    )
                    ...
                    if "isKey" in prop and prop["isKey"] == "true":
                        feature_table.register_entity(
                            feast.Entity(
                                name=name,
                                join_keys=[name],
                                description=name,
                                value_type=FeatureType.from_str_to_entity_type(prop["type"]),
                            )
                        )
                    else:
                        if "format" in prop and prop["format"] == "date-time":
                            continue
                        feature_table.register_features(
                            feast.Field(name=name, dtype=FeatureType.from_str_to_feature_type(prop["type"]))
                        )

                if len(list(filter(lambda x: x["name"] == feature_table.created_col, table_schema))) == 0:
                    table_schema.append(
                        {
                            "name": feature_table.created_col,
                            "type": FeatureType.from_str_to_bq_type("string", format="date-time").name,
                        }
                    )
                    pd_schema[feature_table.created_col] = pd.Series(
                        dtype=FeatureType.from_str_to_pd_type("string", format="date-time")
                    )
                if len(list(filter(lambda x: x["name"] == feature_table.evt_col, table_schema))) == 0:
                    table_schema.append(
                        {
                            "name": feature_table.evt_col,
                            "type": FeatureType.from_str_to_bq_type("string", format="date-time").name,
                        }
                    )
                    pd_schema[feature_table.evt_col] = pd.Series(
                        dtype=FeatureType.from_str_to_pd_type("string", format="date-time")
                    )
    ...
    def get_last_row(
        ...
        date_from: typing.Optional[datetime] = None,
        ...
        return pd.read_sql(f"SELECT {feature_table.evt_col} FROM {feature_table.name} {_where} {_order_by}", conn)
