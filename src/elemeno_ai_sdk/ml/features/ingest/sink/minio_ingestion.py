...
from elemeno_ai_sdk.config import Configs, logging
...
class IngestionParams:
    def __init__(
        ...
        dest_folder_col: Optional[str] = None,
        media_name_col: Optional[str] = None,
...
class DigestionParams:
    def __init__(
        ...
        remote_folder_col: Optional[str] = None,
        media_name_col: Optional[str] = None,
        minio_bucket: str = "elemeno-cos",
    ):
        self.media_path_col = media_path_col
...
class MinioIngestion(FileIngestion):
    ...
    def io_batch_ingest_from_df(
        self, feature_table_name: str, to_ingest: pd.DataFrame, media_columns: List[MediaColumn]
    ):
        ...
        for col in local_path_cols:
            if col and col.is_upload:
                ...
                pool = Pool(cpu_count())
                raw = map(
                    lambda x: IngestionParams(
                        media_path_col=col.name,
                        to_ingest=x,
                        dest_folder=dest_folder_name,
                        minio_bucket=self.config.cos.bucket,
                    ),
                    df_dict,
                )
        ...
        for col in remote_path_cols:
            if col and not col.is_upload:
                logging.info(
                    "Downloading {} files from media column {} to the remote persistence".format(len(df_dict), col)
                )
                pool = Pool(cpu_count())
                raw = map(
                    lambda x: IngestionParams(
    ...
    def io_batch_ingest(self, to_ingest: List[Dict]):
        ...
        raw = map(
            lambda x: IngestionParams(
                config.feature_store.source.params.binary.media_id_col,
                config.feature_store.source.params.binary.media_url_col,
    ...
    def download_file_from_remote(self, p: "DigestionParams"):
        ...
        try:
            ...
            logging.error(
                "Error downloading file {} from bucket {} and folder {}. Error: {}".format(
                    file_path, bucket, folder_remote, e
                )
            )
        return None
            ...
            if content_type.startswith("image"):
                ...
                try:
                    self.client.put_object(
                        "elemeno-cos", f"binary_data_parallel/{folder_id}/{position}_{media_id}.{media_extension}", st
                    )
                    logging.error(
                        "uploaded path: " + f"binary_data_parallel/{folder_id}/{position}_{media_id}.{media_extension}"
                    )
                except Exception as e:
