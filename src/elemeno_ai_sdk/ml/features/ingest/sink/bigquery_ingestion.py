...
class BigQueryIngestion(Ingestion):
    ...
    def ingest(
        ...
        schema: typing.List[typing.Dict] = None,
    ) -> None:
    ...
    def ingest_schema(self, feature_table: FeatureTable, schema_file_path: str) -> str:
        ...
        with open(schema_file_path, mode="r", encoding="utf-8") as schema_file:
            ...
            for name, prop in jschema["properties"].items():
                ...
                if "isKey" in prop and prop["isKey"] == "true":
                    feature_table.register_entity(
                        feast.Entity(
                            name=name, description=name, value_type=FeatureType.from_str_to_feature_type(prop["type"])
                        )
                    )
                else:
                    ...
                    feature_table.register_features(
                        feast.Feature(name, FeatureType.from_str_to_feature_type(prop["type"]))
                    )
            ...
            location = self._fs.config.offline_store.location
            dataframe.to_gbq(
                destination_table=f"{dataset}.{feature_table.name}",
                project_id=project_id,
                if_exists="append",
                location=location,
            )
