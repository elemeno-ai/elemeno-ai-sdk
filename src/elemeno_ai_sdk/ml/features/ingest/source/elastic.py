...
class ElasticIngestionSource(BaseSource):
    def __init__(self, host: str, username: str, password: str):
        super().__init__()
        self._es = Elasticsearch(hosts=[host], http_auth=(username, password))
    ...
    def read(
        ...
        media_id_col: Optional[str] = None,
        ...
        if count <= max_per_page:
            ...
            self._add_to_prepard_medias(
                binary_columns, media_id_col, dest_folder_col, res, prepared_medias=binary_prepared
            )
        ...
        if pages > 10:
            logger.warning(
                "More than 10 pages, will limit to 10 pages and you need to repeat the operation to get the rest"
            )
            pages = 10
    ...
    def _add_to_prepard_medias(
        self, binary_columns: List[str], media_id_col: str, dest_folder_col: str, res: Dict, prepared_medias: List = []
    ):
