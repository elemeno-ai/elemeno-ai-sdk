...
class Query:
    def __init__(self, feature_store: BaseFeatureStore, definition: FeatureTable):
    ...
    def get_historical_features(
        self, entities_where: pd.DataFrame, features_selected: typing.List[str] = None
    ) -> pd.DataFrame:
    ...
    def get_online_features(self, entity_keys: typing.List[typing.Dict[str, typing.Any]]) -> pd.DataFrame:
        ...
        for t in entity_keys:
            for k, _ in t.items():
                if k not in ents:
                    raise ValueError(
                        f"The key {k} is invalid, it's not part of the entities in the feature table definition"
                    )
